...
if LDPC_bitlength == 64800 %如果LDPC编码长度为64800则使用DVB-S2标准
    ...
    H1 =sparse(buildH( LDPC_bitlength, R1));
end
%初始化固定码率的LDPC矩阵
...
%初始化其他码率的矩阵
ldpcEncoder1 = comm.LDPCEncoder(H1);
ldpcDecoder1 = comm.LDPCDecoder(H1);
...
for ii = 1:length(snr)
    ...
    parfor k = 1:nSim
        ...
        noise=randn(1,n_ASK).';
        %AWGN
        Rx_QAM_Uniform = out_Tx_I_Uniform.' + Standard_variance1_Uniform.*noise;
        ...
        for j = 1:n
            %I路各比特位LLR
            %随机
            llr_I_Uniform(:,j) = log(P(Rx_I_Uniform,ASK(Gray_table(j,:)==0),Standard_variance1_Uniform,ones(1,M_ASK/2)/M_ASK)./...
                P(Rx_I_Uniform,ASK(Gray_table(j,:)==1),Standard_variance1_Uniform,ones(1,M_ASK/2)/M_ASK));
